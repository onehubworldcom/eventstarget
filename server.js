...
function weekendRange(){
  ...
  // If it is already Saturday or Sunday, use the current weekend; otherwise use the next one.
  const daysToSaturday=day===0?-1:day===6?0:6-day;
  start.setDate(start.getDate()+daysToSaturday);
  const end=new Date(start); end.setDate(end.getDate()+2); // Monday 00:00, exclusive
  return {
    startDateTime:isoDate(start)+'T00:00:00Z',
    endDateTime:isoDate(end)+'T00:00:00Z',
    startDate:isoDate(start),
    endDate:isoDate(end)
  };
}
function cleanKeyword(text){
  return text.replace(/\b(free|events?|event|this|the|on|this weekend|weekend)\b/gi,' ').replace(/\s+/g,' ').trim();
}
...
app.get('/api/events',async(req,res)=>{
  try{
    ...
    let events=(data._embedded?.events||[]).map(e=>{
      ...
      return {
        id:e.id||e.url||e.name,
        name:e.name,
        date:e.dates?.start?.localDate||'',
        time:e.dates?.start?.localTime||'',
        url:e.url||'',
        image:(e.images||[]).slice().sort((a,b)=>(b.width||0)*(b.height||0)-(a.width||0)*(a.height||0))[0]?.url||'',
        venue:e._embedded?.venues?.[0]?.name||'',
        city:e._embedded?.venues?.[0]?.city?.name||'',
        priceMin:typeof pr?.min==='number'?pr.min:null,
        priceMax:typeof pr?.max==='number'?pr.max:null,
        currency:pr?.currency||'',
        price:pr?`${pr.currency||'USD'} ${pr.min}${pr.max!==pr.min?'–'+pr.max:''}`:'See official listing',
        type:e.classifications?.[0]?.segment?.name||'Event'
      };
    }).filter(e=>{
      if(seen.has(e.id)) return false;
      seen.add(e.id);
      if(range && (!e.date || e.date<range.startDate || e.date>=range.endDate)) return false;
      if(isFree && e.priceMin!==0 && e.priceMax!==0) return false;
      return true;
    }).slice(0,30).map(({id,priceMin,priceMax,currency,...event})=>event);
