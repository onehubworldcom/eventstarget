...
function cleanKeyword(text){
  return String(text||'').replace(/\b(free|events?|this|the|on|near|in|today|tonight|weekend|upcoming|january|february|march|april|may|june|july|august|september|october|november|december|\d{4}|usa|canada)\b/gi,' ').replace(/\s+/g,' ').trim();
}
...
app.get('/api/events',async(req,res)=>{
  try{
    ...
    if(req.query.city)q.set('city',String(req.query.city).trim());
    if(req.query.classificationName)q.set('classificationName',String(req.query.classificationName).trim());
